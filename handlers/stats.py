...
async def stats_handler(message: Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при получении статистики.", reply_markup=get_main_keyboard())

@router.message(lambda message: message.text == "📊 Получить статус")
async def stats_button_handler(message: Message):
    if message.from_user.id not in config.allowed_users:
        return
    logger.info(f"Получен запрос по кнопке статистики от пользователя {message.from_user.id}")
    try:
        disks = SystemInfoService.get_disks_info()
        cpu = SystemInfoService.get_cpu_info()
        mem = SystemInfoService.get_memory_info()
        ...
        text = format_system_stats(disks, cpu, mem, net)
        await message.answer(text, parse_mode="HTML", reply_markup=get_main_keyboard())
        logger.info(f"Успешно отправлена статистика пользователю {message.from_user.id} (кнопка)")
    except Exception as e:
        logger.exception("Ошибка при обработке кнопки статистики")
        await message.answer("Произошла ошибка при получении статистики.", reply_markup=get_main_keyboard()) 